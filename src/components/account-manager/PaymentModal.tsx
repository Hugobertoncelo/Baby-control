...
const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const pricingPlans: PricingPlan[] = [
    {
      id: "monthly",
      name: "Monthly",
      description: "Perfect for trying out Baby Control",
      price: 2.0,
      interval: "month",
  ...
  const handleSelectPlan = async (plan: PricingPlan) => {
    ...
    try {
      const authToken = localStorage.getItem("authToken");
      const response = await fetch(
        "/api/accounts/payments/create-checkout-session",
        {
          method: "POST",
          headers: {
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      const authToken = localStorage.getItem("authToken");
      const response = await fetch(
        "/api/accounts/payments/cancel-subscription",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const renderSubscriptionManagement = () => {
    ...
    return (
      <div className="space-y-4">
        <div
          className={cn(
            "bg-gradient-to-r from-teal-50 to-blue-50 border border-teal-200 rounded-lg p-6",
            "payment-modal-active-subscription"
          )}
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h4
                className={cn(
                  "text-lg font-semibold text-teal-800 mb-2",
                  "payment-modal-subscription-title"
                )}
              >
                Active{" "}
              ...
              {accountStatus.planType === "sub" &&
                subscriptionStatus.currentPeriodEnd && (
                  <div className="space-y-2 text-sm">
                    <div
                      className={cn(
                        "flex items-center gap-2 text-teal-700",
                        "payment-modal-subscription-info"
                      )}
                    >
                    ...
                    {subscriptionStatus.paymentMethod && (
                      ...
                      >
                        ...
                        <span>
                          {subscriptionStatus.paymentMethod.brand.toUpperCase()}{" "}
                          ending in {subscriptionStatus.paymentMethod.last4}
                        </span>
                      </div>
                    )}
                    ...
                    {subscriptionStatus.cancelAtPeriodEnd && (
                      <div
                        className={cn(
                          "mt-3 p-3 bg-amber-50 border border-amber-200 rounded-md",
                          "payment-modal-cancelled-warning"
                        )}
                      >
                        <div
                          className={cn(
                            "flex items-center gap-2 text-amber-700",
                            "payment-modal-cancelled-warning-text"
                          )}
                        >
                        ...
                        <p
                          className={cn(
                            ...
                            "payment-modal-cancelled-warning-description"
                          )}
                        >
              ...
              {accountStatus.planType === "full" && (
                <p
                  className={cn("text-teal-700", "payment-modal-lifetime-text")}
                >
                  You have lifetime access to all features. Thank you for your
                  support!
                </p>
              )}
            </div>
        ...
        {accountStatus.planType === "sub" && (
          <div className="space-y-4">
            ...
            <div
              className={cn(
                "bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-lg p-4",
                "payment-modal-upgrade-section"
              )}
            >
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1">
                  <h5
                    className={cn(
                      "font-semibold text-purple-800 mb-1",
                      "payment-modal-upgrade-title"
                    )}
                  >
                  ...
                  <p
                    className={cn(
                      "text-sm text-purple-700 mb-2",
                      "payment-modal-upgrade-description"
                    )}
                  >
                    Get lifetime access for a one-time payment of $12. No more
                    recurring charges!
                  </p>
                  <ul
                    className={cn(
                      "text-xs text-purple-600 space-y-1 mb-3",
                      "payment-modal-upgrade-list"
                    )}
                  >
                    <li>• Your subscription will be automatically cancelled</li>
                    <li>
                      • You'll maintain access for the remainder of your current
                      billing period
                    </li>
              ...
              <Button
                onClick={() =>
                  handleSelectPlan(
                    pricingPlans.find((p) => p.interval === "lifetime")!
                  )
                }
                disabled={loading}
                size="sm"
              ...
              <div className="mb-4">
                ...
                <p
                  className={cn(
                    ...
                    "payment-modal-plan-description"
                  )}
                >
                  {plan.description}
                </p>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            >
              ...
              <p
                className={cn(
                  ...
                  "payment-modal-error-description"
                )}
              >
                {error}
              </p>
        ...
        <div
          className={cn(
            "flex justify-end pt-4 border-t border-gray-400",
