...
const FamilyPeopleTab: React.FC<FamilyPeopleTabProps> = ({
  ...
  const [selectedCaretaker, setSelectedCaretaker] =
    useState<CaretakerData | null>(null);
  ...
  const fetchFamilyPeople = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load family people data"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const calculateAge = (birthDate: Date): string => {
    ...
    if (diffDays < 30) {
      return `${diffDays} dias de idade`;
    } else if (diffDays < 365) {
      const months = Math.floor(diffDays / 30);
      return `${months} mês${months > 1 ? "s" : ""} idade`;
    } else {
      const years = Math.floor(diffDays / 365);
      const remainingMonths = Math.floor((diffDays % 365) / 30);
      return `${years} ano${years > 1 ? "s" : ""} ${
        remainingMonths > 0
          ? `${remainingMonths} mês${remainingMonths > 1 ? "s" : ""}`
          : ""
      } idade`.trim();
    }
  };
  ...
  if (isLoading) {
    return (
      ...
      >
        ...
        <p className={cn("mt-2 text-gray-600", "account-manager-loading-text")}>
          Carregando pessoas da família...
        </p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div
        className={cn(styles.errorContainer, "account-manager-error-container")}
      >
        <div className="flex items-center gap-2 text-red-600 mb-2">
          <AlertTriangle className="h-5 w-5" />
          <p className="font-medium">Erro</p>
        </div>
        <p className={cn("text-red-500 mb-4", "account-manager-error-text")}>
          {error}
        </p>
        <Button variant="outline" onClick={fetchFamilyPeople} className="mt-2">
          Tentar novamente
        </Button>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="outline" size="sm" onClick={handleAddBaby}>
        ...
        {babies.length > 0 ? (
          ...
          >
            {babies.map((baby) => (
              <div
                key={baby.id}
                className={cn(styles.card, "account-manager-card")}
              >
                <div
                  className={cn(
                    styles.cardHeader,
                    "account-manager-card-header"
                  )}
                >
                  <div>
                    <h4
                    ...
                    {baby.inactive && (
                      <span
                        className={cn(
                          ...
                          "account-manager-badge-inactive"
                        )}
                      >
                        Inativo
                      </span>
          ...
          <div className={cn(styles.emptyState, "account-manager-empty-state")}>
            ...
            <p>Nenhum bebê adicionado ainda</p>
            <p className="text-sm">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="outline" size="sm" onClick={handleAddCaretaker}>
            <Plus className="h-4 w-4 mr-2" />
            Adicionar zelador
          </Button>
        </div>
        ...
        {caretakers.length > 0 ? (
          ...
          <div className={cn(styles.emptyState, "account-manager-empty-state")}>
            ...
            <p>Nenhum cuidador foi adicionado ainda.</p>
            <p className="text-sm">
              Adicione cuidadores para ajudar a administrar sua família.
            </p>
          </div>
        )}
      </div>
      ...
      >
        ...
        {contacts.length > 0 ? (
          ...
          <div className={cn(styles.emptyState, "account-manager-empty-state")}>
            ...
            <p className="text-sm">
              Adicione contatos como médicos, familiares ou cuidadores.
            </p>
          </div>
        )}
      </div>
