...
export default function SettingsForm({
    ...
    try {
      ...
      if (authToken) {
        try {
          ...
          console.error("Erro ao analisar o token JWT em SettingsForm:", error);
        }
      }
      ...
      const settingsUrl =
        isSysAdmin && familyId
          ...
          : "/api/settings";
      const babiesUrl =
        isSysAdmin && familyId ? `/api/baby?familyId=${familyId}` : "/api/baby";
      const caretakersUrl =
        isSysAdmin && familyId
          ? `/api/caretaker?includeInactive=true&familyId=${familyId}`
          : "/api/caretaker?includeInactive=true";
      const contactsUrl =
        isSysAdmin && familyId
          ? `/api/contact?familyId=${familyId}`
          : "/api/contact";
        ...
        if (settingsData.data?.authType) {
          ...
          if (willHaveCaretakers) {
            ...
            if (caretakersData.success) {
              caretakerData = caretakersData.data.filter(
                (c: any) => c.loginId !== "00" && !c.deletedAt
              );
            }
          }
    ...
    try {
      ...
      if (authToken) {
        try {
          ...
          console.error(
            "Erro ao analisar o token JWT em handleSettingsChange:",
            error
          );
        }
      }
        ...
        if (response.status === 403) {
          ...
          if (errorData) {
            showToast({
              variant: "error",
              title: "Erro",
              message: errorData.error || "Falha ao atualizar configurações",
              duration: 5000,
            });
            return;
          }
      ...
      if (data.success) {
        ...
        showToast({
          variant: "error",
          title: "Erro",
          message: data.error || "Falha ao atualizar configurações",
          duration: 5000,
        });
      }
    } catch (error) {
  ...
  const handleFamilySave = async () => {
    ...
    if (!familyEditData.name || !familyEditData.slug) {
      alert("Nome da família e slug são obrigatórios");
      return;
    }
    ...
    try {
        ...
        if (response.status === 403) {
          ...
          if (errorData) {
            showToast({
              variant: "error",
              title: "Erro",
              message: errorData.error || "Falha ao salvar alterações",
              duration: 5000,
            });
            return;
          }
      ...
      if (data.success) {
        ...
        showToast({
          variant: "error",
          title: "Erro",
          message: data.error || "Falha ao salvar alterações",
          duration: 5000,
        });
      }
    } catch (error) {
  ...
  return (
    <>
      <FormPage
        ...
        title="Configurações"
        description="Configure suas preferências para o aplicativo Baby Tracker"
      >
        <FormPageContent>
          <div className="space-y-6">
              ...
              <div>
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="relative">
                        <Input
                          ...
                          placeholder="Digite o slug da família"
                          className={`w-full ${
                ...
                {!editingFamily && (
                  <p className="text-sm text-gray-500 mt-1">
                    Este é o identificador único da URL da sua família
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4 border-t border-slate-200 pt-6">
              <h3 className="form-label mb-4">Configurações de Autenticação</h3>

              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  <span className="text-sm text-gray-500">PIN do Sistema</span>
                  <Switch
                    checked={localAuthType === "CARETAKER"}
                    onCheckedChange={(checked) =>
                      handleAuthTypeChange(checked ? "CARETAKER" : "SYSTEM")
                    }
                    disabled={loading}
                    variant="green"
                  />
                ...
                <div>
                  <p className="text-sm text-gray-500">
                    {localAuthType === "CARETAKER"
                      ? "Use IDs de login individuais e PINs para cuidadores"
                      : "Use PIN do sistema compartilhado para todos os usuários"}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                {localAuthType === "CARETAKER" ? (
                  <p className="text-sm text-red-500 mt-1">
                    O PIN do sistema está desativado quando se usa autenticação
                    de cuidador.
                  </p>
                ) : (
                  <p className="text-sm text-gray-500 mt-1">
              ...
              <div className="mb-4">
                ...
                {localAuthType === "SYSTEM" && (
                  <p className="text-sm text-red-500 mt-1">
                    Logins de cuidadores estão desativados no modo PIN do
                    sistema
                  </p>
                )}
              </div>
            ...
            <div className="border-t border-slate-200 pt-6">
              <h3 className="form-label mb-4">Gerenciar Contatos</h3>
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-2 w-full">
                  <div className="flex-1 min-w-[200px]">
                    <Select
            ...
            <div className="border-t border-slate-200 pt-6">
              <h3 className="form-label mb-4">Configurações de Depuração</h3>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="form-label">
                      Ativar Temporizador de Sessão de Depuração
                    </Label>
                    <p className="text-sm text-gray-500">
                      Mostra a expiração do token JWT e o tempo de inatividade
                      do usuário
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleSettingsChange({
                          enableDebugTimer: e.target.checked,
                        } as any)
                      }
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="form-label">
                      Ativar Ferramenta de Fuso Horário de Depuração
                    </Label>
                    <p className="text-sm text-gray-500">
                      Mostra informações de fuso horário e status de horário de
                      verão
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleSettingsChange({
                          enableDebugTimezone: e.target.checked,
                        } as any)
                      }
                      className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleSettingsChange({ defaultBottleUnit: value })
                      }
                      disabled={loading}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleSettingsChange({ defaultSolidsUnit: value })
                      }
                      disabled={loading}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleSettingsChange({ defaultHeightUnit: value })
                      }
                      disabled={loading}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleSettingsChange({ defaultWeightUnit: value })
                      }
                      disabled={loading}
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        {units
                          .filter((unit) =>
                            ["LB", "KG", "G"].includes(unit.unitAbbr)
                          )
                          .map((unit) => (
                            <SelectItem
                              key={unit.unitAbbr}
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleSettingsChange({ defaultTempUnit: value })
                      }
                      disabled={loading}
                    >
                      <SelectTrigger>
              ...
              <div className="border-t border-slate-200 pt-6">
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    Abrir Gerenciador de Famílias
                  </Button>
                  <p className="text-sm text-gray-500">
                    Acesse a gestão de famílias em todo o sistema e
