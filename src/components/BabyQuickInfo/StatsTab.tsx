    ...
    const fetchActivities = async () => {
      ...
      try {
        ...
        const url = `/api/timeline?babyId=${
          ...
          startDate
        )}&endDate=${encodeURIComponent(endDate)}&_t=${timestamp}`;
    ...
    const mostCommonUnit =
      Object.keys(feedUnits).length > 0
        ? Object.entries(feedUnits).reduce((a, b) =>
            feedUnits[a[0]] > feedUnits[b[0]] ? a : b
          )[0]
        : "oz";
      ...
      <div className="space-y-2">
        <div>
          ...
          <div className="flex space-x-1 overflow-x-auto">
            {(["2dia", "7dia", "14dia", "30dia"] as const).map((period) => (
              <Button
                key={`main-${period}`}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-4 mt-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          ...
          <CardVisual
            ...
            trend={
              mainStats.avgNightSleepTime >= compareStats.avgNightSleepTime
                ? "positive"
                : "negative"
            }
          />
          ...
          <CardVisual
            ...
            comparativeValue={
              compareStats.avgFeedAmount.toFixed(1) +
              " " +
              compareStats.feedUnit
            }
            trend={
