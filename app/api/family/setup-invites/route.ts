...
async function getHandler(
  req: NextRequest
): Promise<NextResponse<ApiResponse<FamilySetupInvite[]>>> {
  try {
    ...
    console.error("Erro ao buscar convites de configuração de família:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Falha ao buscar convites de configuração de família",
      },
      { status: 500 }
    );
  }
...
async function deleteHandler(
  ...
  try {
    ...
    console.error("Erro ao deletar convite de configuração de família:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Falha ao deletar convite de configuração de família",
      },
      { status: 500 }
    );
  }
