  ...
  try {
    ...
    if (!isAccountAuth || !accountId || !familyId) {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          error: "Autenticação da conta e associação à família obrigatórias",
        },
        { status: 403 }
      );
    }
    ...
    const [
      ...
      prisma.calendarEvent.findMany({
        ...
        include: {
          ...
          caretakers: { include: { caretaker: { select: { name: true } } } },
          contacts: {
            include: { contact: { select: { name: true, role: true } } },
          },
        },
      }),
    ...
    const exportData = {
      ...
      calendarEvents: calendarEvents.map((e) => ({
        ...e,
        associatedBabies: e.babies
          .map((b) => `${b.baby.firstName} ${b.baby.lastName}`)
          .join("; "),
        associatedCaretakers: e.caretakers
          .map((c) => c.caretaker.name)
          .join("; "),
        associatedContacts: e.contacts
          .map((c) => `${c.contact.name} (${c.contact.role})`)
          .join("; "),
        babies: undefined,
    ...
    return new NextResponse(new Uint8Array(dataExport.buffer), {
      status: 200,
      headers: {
    ...
    return NextResponse.json<ApiResponse<null>>(
      { success: false, error: "Falha ao criar exportação de dados" },
      { status: 500 }
    );
  }
}
