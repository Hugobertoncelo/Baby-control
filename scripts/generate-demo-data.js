...
async function createDemoCaretaker(family) {
  const firstName = randomChoice(femaleFirstNames);
...
async function createDemoBabies(family, sourceBabies) {
  ...
  if (sourceBabies.length > 0) {
    const sourceBaby = sourceBabies[0];
  ...
  for (const mapping of dateMapping) {
    for (const babyMapping of babyMappings) {
      if (Math.random() > 0.2) {
        ...
        bathTime.setHours(19, randomInt(-60, 60), randomInt(0, 59));
...
async function generateDemoData() {
  try {
    ...
    console.log(`Target period: Last 30 Dias (${dateMapping[0].targetDate.toLocaleDateString()} to ${dateMapping[dateMapping.length - 1].targetDate.toLocaleDateString()})`);
